...
func TestRunClaudeOnce_ArgsWithModel(t *testing.T) {
	...
	// then
	expected := []string{"--model", "opus", "--verbose", "--output-format", "stream-json", "--dangerously-skip-permissions", "--print", "-p", "Analyze these issues"}
...
func TestRunClaudeOnce_ArgsWithoutModel(t *testing.T) {
	...
	// then
	expected := []string{"--verbose", "--output-format", "stream-json", "--dangerously-skip-permissions", "--print", "-p", "test prompt"}
	if len(capturedArgs) != len(expected) {
		t.Fatalf("expected %d args, got %d: %v", len(expected), len(capturedArgs), capturedArgs)
	}
	for i, e := range expected {
