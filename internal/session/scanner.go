...
func RunScan(ctx context.Context, cfg *domain.Config, baseDir string, sessionID string, dryRun bool, out io.Writer, runner port.ClaudeRunner, onceRunner port.ClaudeRunner, logger domain.Logger) (*domain.ScanResult, error) {
	...
	classifyCtx, classifySpan := platform.Tracer.Start(ctx, "classify") // nosemgrep: adr0003-otel-span-without-defer-end -- End() called per branch [permanent]
	classifyOutput := filepath.Join(scanDir, "classify.json")

	classifyPrompt, err := harness.RenderClassifyPrompt(cfg.Lang, domain.ClassifyPromptData{
	...
	deepscanCtx, deepscanSpan := platform.Tracer.Start(ctx, "deepscan") // nosemgrep: adr0003-otel-span-without-defer-end -- End() called at line 208 [permanent]
	...
	deepScanFn := func(ctx context.Context, cfg *domain.Config, scanDir string, index int, cluster domain.ClusterScanResult) (domain.ClusterScanResult, error) {
		...
		for j, chunk := range chunks {
			...
			prompt, renderErr := harness.RenderDeepScanPrompt(cfg.Lang, domain.DeepScanPromptData{
				ClusterName:     cc.Name,
...
func generateWaveForCluster(ctx context.Context, cfg *domain.Config, scanDir string, index int, cluster domain.ClusterScanResult, dryRun bool, linearTools RunOption, runner port.ClaudeRunner, logger domain.Logger) (domain.WaveGenerateResult, error) {
	...
	prompt, err := harness.RenderWaveGeneratePrompt(cfg.Lang, domain.WaveGeneratePromptData{
