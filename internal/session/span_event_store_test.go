...
func TestSpanEventStore_AlwaysCollectsAllAttributes(t *testing.T) {
	...
	if !hasAttribute(appendSpan, "event.count.in") {
		t.Error("attribute event.count.in missing")
	}
	if !hasAttribute(appendSpan, "event.append.bytes") {
		t.Error("attribute event.append.bytes missing")
	}
	...
	if !hasAttribute(loadSpan, "event.count.out") {
		t.Error("attribute event.count.out missing")
	}
	if !hasAttribute(loadSpan, "event.file.count") {
		t.Error("attribute event.file.count missing")
	}
	if !hasAttribute(loadSpan, "event.corrupt_line.count") {
		t.Error("attribute event.corrupt_line.count missing")
	}
}
