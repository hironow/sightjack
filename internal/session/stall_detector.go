...
type StallDetector struct { // nosemgrep: structure.multiple-exported-structs-go -- structure category drained in apr29-structure sweep; cohesive type family co-location is intentional [permanent]
...
type StallResult struct { // nosemgrep: domain-primitives.public-string-field-go -- internal result DTO [permanent]
	Detected    bool
	WaveID      string
	ClusterName string
	Fingerprint string
