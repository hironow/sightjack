...
type ClaudeAdapter struct { // nosemgrep: domain-primitives.public-string-field-go -- adapter config struct; ToolName is a config identifier
	ClaudeCmd  string
	Model      string
	TimeoutSec int
	...
	go func() {
		...
		for _, msg := range messages {
			...
			case "assistant":
				...
				if err == nil && text != "" {
					if w != nil {
						if _, wErr := w.Write([]byte(text)); wErr != nil && a.Logger != nil {
							a.Logger.Info("stream write: %v", wErr)
						}
					}
					output.WriteString(text)
				}
				tools, toolErr := msg.ExtractToolUse()
				if toolErr == nil {
					for _, t := range tools {
						if a.Logger != nil {
				...
				am, amErr := msg.ParseAssistantMessage()
				if amErr != nil && a.Logger != nil {
					a.Logger.Info("ParseAssistantMessage: %v", amErr)
				}
				if am != nil {
					if am.Model != "" {
