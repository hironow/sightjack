...
func TestIsWaveApplyComplete_NoErrors(t *testing.T) {
	...
	// when
	complete := session.IsWaveApplyComplete(result)
...
func TestBuildCompletedWaveMap(t *testing.T) {
	...
	completed := session.BuildCompletedWaveMap(waves)
	if len(completed) != 2 {
...
func TestBuildWaveStates(t *testing.T) {
	...
	states := session.BuildWaveStates(waves)
	if len(states) != 2 {
...
func TestBuildCompletedWaveMap_Empty(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// when
			completed := session.BuildCompletedWaveMap(tt.waves)
...
func TestBuildWaveStates_Empty(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// when
			states := session.BuildWaveStates(tt.waves)
...
func TestApplyModifiedWave_PreservesIdentity(t *testing.T) {
	...
	result := session.ApplyModifiedWave(original, modified, map[string]bool{})
...
func TestApplyModifiedWave_LocksOnUnmetPrerequisites(t *testing.T) {
	...
	// when
	result := session.ApplyModifiedWave(original, modified, completed)
...
func TestApplyModifiedWave_PropagatesLockToWaves(t *testing.T) {
	...
	result := session.ApplyModifiedWave(waves[0], modified, completed)
	if result.Status != "locked" {
		t.Fatalf("precondition: expected locked, got %s", result.Status)
	}

	// when: propagate back to waves slice
	session.PropagateWaveUpdate(waves, result)
...
func TestMergeCompletedStatus_PreservesCompleted(t *testing.T) {
	...
	// when
	merged := session.MergeCompletedStatus(oldCompleted, newWaves)

	// then: auth-w1 should be completed (was in old)
	for _, w := range merged {
		if session.WaveKey(w) == "Auth:auth-w1" && w.Status != "completed" {
...
func TestRestoreWaves_ConvertsWaveStatesToWaves(t *testing.T) {
	...
	// when
	waves := session.RestoreWaves(states)
...
func TestCalcNewlyUnlocked_CompletingWaveUnlocksOne(t *testing.T) {
	...
	// when
	got := session.CalcNewlyUnlocked(oldAvailable, newAvailable)
...
func TestPartialApplyDelta(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// when
			got := session.PartialApplyDelta(result, delta)
...
func TestCheckCompletenessConsistency(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := session.CheckCompletenessConsistency(tt.overall, tt.clusters)
			if got != tt.wantWarn {
...
func TestMergeOldWaves_CarriesForwardFailedClusters(t *testing.T) {
	...
	merged := session.MergeOldWaves(oldWaves, newWaves, scannedClusters, failedNames)
