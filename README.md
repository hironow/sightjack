...
```
cmd/sightjack/          CLI entry point
internal/
  cmd/                  Cobra commands (scan, run, waves, select, discuss, apply, etc.)
  usecase/              Business logic (PolicyEngine + handlers)
  session/              I/O orchestration (Claude, Linear, D-Mail, gates)
  eventsource/          Event persistence (JSONL append-only)
  domain/               Pure domain types (projection, wave scheduling)
