...
func TestE2E_Gate_AutoApproveWithConvergence(t *testing.T) {
	...
	cmd := exec.Command(sightjackBin(), "run", "--linear", "--auto-approve", "--idle-timeout=-1s", dir)
...
func TestE2E_Gate_ApproveCmdDenies(t *testing.T) {
	...
	cmd := exec.Command(sightjackBin(), "run", "--linear", "--approve-cmd", "exit 1", "--idle-timeout=-1s", dir)
	var stdout, stderr bytes.Buffer
	cmd.Stdout = &stdout
	cmd.Stderr = &stderr
	err := cmd.Run()
...
func TestE2E_Gate_NotifyCmdInvoked(t *testing.T) {
	...
	cmd := exec.Command(sightjackBin(), "run", "--linear", "--auto-approve", "--idle-timeout=-1s", "--notify-cmd", notifyCmd, dir)
	var stdout, stderr bytes.Buffer
	cmd.Stdout = &stdout
	cmd.Stderr = &stderr
	err := cmd.Run()
